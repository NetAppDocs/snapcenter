...
- **Azure NetApp Files:** A file storage service that customers use with SnapCenter to protect SQL Server, SAP HANA, and Oracle workloads. Always use the full name "Azure NetApp Files"—do not abbreviate to "ANF" even though this abbreviation might appear in some existing documentation. When referring to Azure NetApp Files resources in context, use the full name on first reference, then "Azure NetApp Files" (not "ANF") in subsequent references.
