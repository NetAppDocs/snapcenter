...
- **NetApp supported plug-ins:** A SnapCenter component that provides a framework for plug-ins not included in the dedicated plug-in offerings. The framework ships with several pre-built plug-ins (MongoDB, Oracle Applications, SAP ASE, SAP MaxDB, Storage) and can accommodate custom-developed plug-ins. This is part of SnapCenter, not a separate NetApp product.
