...
. In the Resources page, select *Add IBM DB2 Resource*.
...
Mount paths
a| 
Enter the mount paths where the resource is mounted. This is only applicable to Windows hosts.
a|
Credentials
a|
...
. In the Provide Storage Footprint page, select a storage type and choose one or more volumes, LUNs, and qtrees, and then select *Save*.
+
Optional: You can select the *image:../media/add_policy_from_resourcegroup.gif[add policy form resource group]* icon to add more volumes, LUNs, and qtrees from other storage systems.
