...
. Optional: In the Resource Settings page, for resources on Linux hosts where the SnapCenter Plug-in for IBM Db2 is installed, enter custom key-value pairs for the IBM Db2 plug-in. Custom key-value pairs are not supported for IBM Db2 databases that are registered as a centralized Windows plug-in.
