...
* Ensure that ASP.NET Core Runtime 10.0.7 Hosting Bundle (or later) and PowerShell 7.6.1 (or later) are installed on the Windows plug-in host.
+
Refer to the link:https://library.netapp.com/ecm/ecm_download_file/ECMLP3359467[SnapCenter Software Release Notes^] for the latest qualified versions of ASP.NET Core and PowerShell for your specific SnapCenter version.
* Ensure that Java 17 (Oracle Java, OpenJDK, or SAP JDK) for SnapCenter 6.2.1 and later or Java 11 for SnapCenter 6.2 and earlier, is installed on Windows and Linux plug-in hosts.
