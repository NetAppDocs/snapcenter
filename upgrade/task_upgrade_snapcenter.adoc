...
* During upgrade, the SnapCenter Server host is automatically put into maintenance mode that prevents the server host from running any scheduled jobs. After upgrade, the server host is automatically pulled out of maintenance mode.
