...
. Add the MySQL communication host to SnapCenter Server.
+
----
PS C:> Add-SmHost -HostName 10.232.204.61 -OSType Windows -RunAsName FinanceAdmin -PluginCode mysql
----

. Install the package and the SnapCenter Plug-in for MySQL on the host.
+
For Linux:
+
----
...
+
For Windows, MySQL plug-in will use the default path for SQLLIB folder: “C:\Program Files\IBM\SQLLIB\BIN”
+
