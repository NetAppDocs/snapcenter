 ...
 ** Linux
[NOTE]
 The Plug-in for MySQL has to be installed on the MySQL database server.
...
+
While using the REST API to install Plug-in for MySQL, you must pass the version as 3.0.  For example, MySQL:3.0
...
Installation Path
a|
The Plug-in for MySQL is installed on the MySQL client host, and this host can be on either a Windows system or a Linux system.
...
+
* For Windows plug-in, the install and upgrade logs are located at: _C:\Windows\SnapCenter plugin\Install_<JOBID>\_
* For Linux plug-in, the install logs are located at: _/var/opt/snapcenter/logs/SnapCenter_Linux_Host_Plug-in_Install_<JOBID>.log_ and the upgrade logs are located at: _/var/opt/snapcenter/logs/SnapCenter_Linux_Host_Plug-in_Upgrade_<JOBID>.log_

.After you finish

If you want to upgrade to SnapCenter 6.0 version, the existing PERL-based Plug-in for MySQL will be uninstalled from the remote plug-in server.
