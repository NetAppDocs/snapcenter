...
* Point-in-time and Up-to-minute auto-recovery is not supported.
...
. In the Restore Scope page, select *Complete Resource* to restore the configured data volumes of the MySQL database.
...
+
NOTE: For pre and post commands for quiesce, snapshot, and unquiesce operations, you should check if the commands exist in the command list available on the plug-in host from the _/opt/snapcenter/snapenter/scc/allowed_commands.config_ path for Linux. 
