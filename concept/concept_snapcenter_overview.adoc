...
+
For more information on Windows cmdlets see https://library.netapp.com/ecm/ecm_download_file/ECMLP2886895[SnapCenter Software Cmdlet Reference Guide^].
+
...
You should run the following commands to know the status of the SnapCenter components:

* `systemctl status snapmanagerweb.service`
* `systemctl status scheduler.service`
* `systemctl status smcore.service`
...
SnapCenter Server supports both Microsoft Windows and Linux (RHEL 8, RHEL 9, SLES 15)

If you are using the SnapCenter Plug-ins Package for Linux or the SnapCenter Plug-ins Package for AIX, schedules were executed centrally using the Quartz scheduler.
