...
+
For AIX systems, SAN native devices, filesystems and LVM layouts built on SAN devices are supported.
// [SD]: Updated the above statement and included the below three bullets for BURT 1391312 in 4.5
...
* Supports Enhanced Journaled File System (JFS2) with inline logging on SAN filesystems and LVM layout.
+
...
MySQL, DB2, and MongoDB custom plug-in samples for SnapCenter Software can be downloaded from the https://automationstore.netapp.com/home.shtml[NetApp Storage Automation Store^]. You can also create your own custom plug-ins.

NOTE: MySQL, DB2, and MongoDB custom plug-ins are supported via the NetApp communities only.

NetApp supports the capability to create and use custom plug-ins; however, the custom plug-ins you create are not supported by NetApp. SnapCenter can back up its own repository.
...
The http://mysupport.netapp.com/matrix[NetApp Interoperability Matrix Tool^] (IMT) contains the latest information about the MySQL supported versions.
