...
+
NOTE: Primary SnapLock settings are managed in SnapCenter backup policy and the secondary SnapLock settings are managed by ONTAP SnapLock policy.
...
.. If you want to specify the Snapshot copy locking period, select *Snapshot copy locking period*.
+
The SnapLock retention period must be a number from 1 to 100 years. 
+
The SnapLock retention period must be lesser than or equal to the mentioned retention days.
