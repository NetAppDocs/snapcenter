...
* The retention period prevents the Snapshot copies from being deleted until the SnapLock retention period expires.  This also retains a larger number of Snapshot copies than the count specified in the policy.
...
.. To retain a large number of Snapshot copies than the count specified in the policy, select *Snapshot copy locking period* and specify the locking period. 
