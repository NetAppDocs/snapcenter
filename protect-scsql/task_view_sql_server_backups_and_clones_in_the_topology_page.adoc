...
+
When SnapLock is configured only on the secondary from ONTAP known as SnapLock Vault, clicking the *Refresh* button refreshes the locking period on secondary retrieved from ONTAP.
+
...
+
After failback, to see the status of backups and clones, click *Refresh*.
