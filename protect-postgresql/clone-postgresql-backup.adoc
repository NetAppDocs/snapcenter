...
Clone suffix
a|
Enter a suffix that will be appended to the clone file name to identify that the cluster is a clone.

For example, _db1_clone_. If you are cloning to the same location as the original cluster, you must provide a suffix to differentiate the cloned cluster from the original cluster. Otherwise, the operation fails.
a|
