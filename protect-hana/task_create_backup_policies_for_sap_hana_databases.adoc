...
[IMPORTANT]
If you use the file-based backup to execute an integrity check, you must set the retention count to 1. This enables each of the block integrity checks to overwrite the backup that was created before, and retain only one file-based backup.
a|
