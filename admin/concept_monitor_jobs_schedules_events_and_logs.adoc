...
Stale jobs are created from interruptions in SnapCenter or from improper job updates. Beginning with SnapCenter 6.0.1 release, a predefined schedule has been introduced to clean up these stale jobs that are stuck for more than 72 hours. You can change the schedule frequency by editing the configurable parameter *CleanUpStaleJobsIntervalHours*.
