...
NOTE: This display appears only if you are using SnapCenter Standard capacity-based licenses on Cloud Volumes ONTAP or ONTAP Select platforms. For FAS, AFF, or All SAN Array (ASA) platforms, the SnapCenter license is controller-based and licensed for unlimited capacity, and no capacity license is required.
