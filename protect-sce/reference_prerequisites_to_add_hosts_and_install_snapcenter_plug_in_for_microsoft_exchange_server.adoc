...
* You must be using Microsoft Exchange Server for standalone and Database Availability Group configurations. Refer to the link:https://imt.netapp.com/imt/#search[Interoperability Matrix Tool (IMT)^] for the supported versions of Microsoft Exchange Server.
...
* Ensure the following ports are not blocked in the firewall, otherwise the add host operation fails. To resolve this issue, you must configure the dynamic port range. For more information, see https://docs.microsoft.com/en-us/troubleshoot/windows-server/networking/configure-rpc-dynamic-port-allocation-with-firewalls[Microsoft documentation^]. Refer to the link:https://imt.netapp.com/imt/#search[Interoperability Matrix Tool (IMT)^] for the supported versions of Microsoft Exchange Server.
...
// GH ISSUE #220, 2026-MAR-12
// GH issue #221, 2026-MAY-20