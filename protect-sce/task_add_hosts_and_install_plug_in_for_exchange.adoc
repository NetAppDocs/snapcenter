 ...
 ** Exchange DAG
+
#You can add a DAG by providing the DAG name, DAG IP address, node name, or node IP address.#
+
You can add the IP less DAG cluster by providing the IP address or the FQDN of one of the DAG cluster nodes.
+
#You can add IP less DAG that resides in the same domain or different domain. You can also add multiple IP/IP less DAGs with the same name but different domains.#

#NOTE: For a cross-domain DAG or a standalone host, it is recommended to provide FQDN or the IP address of the host.#
