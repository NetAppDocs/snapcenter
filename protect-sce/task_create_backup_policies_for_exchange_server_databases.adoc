...
. In the Backup Type and Replicationpage, perform the following steps:
 .. Choose backup type:
+
|===
...
NOTE: Select both the secondary replication options for *Secondary snapshot copy locking period* to be effective.
+
You can select *Secondary snapshot copy locking period* in the Retention page.
...
* Select both the secondary replication options.
|===
+
