...
From 6.0 release, a shortcut for PowerShell is created on the desktop. You can directly access the SnapCenter PowerShell cmdlets by using the shortcut.
...
You should have enabled MFA. For information on how to enable MFA,  see link:../install/enable_multifactor_authentication.html[Enable Multi-factor authentication]
