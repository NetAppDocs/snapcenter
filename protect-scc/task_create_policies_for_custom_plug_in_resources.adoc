...
** For ONTAP 9.12.1 and below versions, the clones created from the SnapLock Vault Snapshot copies will inherit the SnapLock Vault expiry time. The storage admin should manually cleanup the clones post the SnapLock expiry time.
...
For more information on SnapLock Vault see Commit Snapshot copies to WORM on a vault
destination
