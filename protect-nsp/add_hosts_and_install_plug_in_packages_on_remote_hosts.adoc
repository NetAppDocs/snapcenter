...
NOTE: The NetApp supported plug-ins can be used in Windows and Linux environments.
...
** For Storage Plug-in, navigate to `_C:\ProgramData\NetApp\SnapCenter\Package Repository_` and select `Storage.zip` folder.
  ...
  ... Select *Upload*.
+
The NetApp supported plug-ins that are uploaded to the SnapCenter Server are listed.
