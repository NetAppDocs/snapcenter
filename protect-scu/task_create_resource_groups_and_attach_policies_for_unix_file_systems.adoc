...
+
NOTE: If you have added different file systems in the resource group, then all volumes from different file systems in the resource group will be put in a Consistency group. Note too when selecting crash consistent backups, by default, even if there is only one filesystem or volume, a consistency (cg) snapshot is taken.
