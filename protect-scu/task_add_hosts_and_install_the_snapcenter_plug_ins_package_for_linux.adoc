...
summary: 'You can use the SnapCenter Add Host page to add hosts, and then install the SnapCenter Plug-ins Package for Linux. The plug-ins are automatically installed on the remote hosts.'
---
...
. In the Select Plug-ins to Install section, select *Unix File Systems*.
...
You can optionally customize the path. If you use the custom path, ensure that the default content of the sudoers is updated with the custom path.
