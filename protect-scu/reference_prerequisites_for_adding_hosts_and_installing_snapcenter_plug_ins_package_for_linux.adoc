...
NOTE: The RSA key is the only SSH supported key format for SnapCenter. Refer to the Knowledge Base article link:../media/ssh_key_creation_steps.adoc[How to create an RSA key for SnapCenter SSH Key Based credential] for details on how to create the SSH key.
...
* You must have installed Java 17 (or Java 11 for SnapCenter 6.2) on your Linux host.
+
NOTE: Ensure that you have installed only the certified edition of  (or Java 11 for SnapCenter 6.2) on the Linux host.
+
...
a|
Java 17 (Oracle Java or OpenJDK) (or Java 11 for SnapCenter 6.2)
