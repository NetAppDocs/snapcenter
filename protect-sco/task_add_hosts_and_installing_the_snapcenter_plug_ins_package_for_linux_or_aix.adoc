...
Host Type
a|
Select *Linux* or *AIX* as the host type.    The SnapCenter Server adds the host, and then installs the Plug-in for Oracle Database and the Plug-in for UNIX if the plug-ins are not already installed on the host.
a|
Host name
a|
...
a|
Select this check box to add all of the cluster nodes in an Oracle RAC.
a|
