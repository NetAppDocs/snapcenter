...
Other hypervisors
a|
Linux running under KVM, Xen, Hyper-V and hyperscaler hypervisors is supported where the guest OS has direct TCP/IP access to the required ONTAP storage resources using the following methods:
...
NOTE: Datastores and raw device presentations are not supported outside VMware.
|===
...
// GH issue #219, 2026-MAR-23 - trying a rebuild
