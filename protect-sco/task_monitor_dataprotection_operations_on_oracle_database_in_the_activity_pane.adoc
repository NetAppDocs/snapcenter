---
permalink: protect-sco/task_monitor_dataprotection_operations_on_oracle_database_in_the_activity_pane.html
sidebar: sidebar
keywords: SnapCenter Server, activity pane, data protection, monitor
summary: 'The Activity pane displays the five most recent operations performed. The Activity pane also displays when the operation was initiated and the status of the operation.'
---
= Monitor data protection operations on Oracle database in the Activity pane
:icons: font
:imagesdir: ../media/

[.lead]
include::../_include/reuse_task_monitor_operations_in_the_activity_pane.adoc[]
