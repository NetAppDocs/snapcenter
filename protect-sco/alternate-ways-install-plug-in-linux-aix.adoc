...
Before installing the plug-in manually, you should validate the signature of the binary package by using the key *snapcenter_public_key.pub* located at _C:\ProgramData\NetApp\SnapCenter\Package Repository_. 

NOTE: Ensure that *OpenSSL 1.0.2k-fips* is installed on the host where you want to install the plug-in.
