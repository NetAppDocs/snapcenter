...
+
You can add additional parameters by clicking *image:../media/add_policy_from_resourcegroup.gif[add policy form resource group]*.
+
...
== Update the preferred IP on the host

After the clone operation completes, the path provided by the Storage Access Layer (SAL) to the clone will be in _<nfs_lif_IP>:<JunctionPath>_ format.  To provide the preferred IP,  you must  configure it on the host using the SCCLI commands.

.Steps
. Login to the database host.
. Initiate a PowerShell connection session with SnapCenter, for a specified user.
+
Open-SmConnection 
. Create an empty file.
+
touch /var/opt/snapcenter/scu/etc/storagepreference.properties
. Configure the preferred data LIF for the SVM.
+
Add-SvmPreferredDataPath -SVM <SVM Name> -DataPath <IP address or FQDN>
. Verify the preferred path.
+
Get-SvmPreferredDataPath to confirm