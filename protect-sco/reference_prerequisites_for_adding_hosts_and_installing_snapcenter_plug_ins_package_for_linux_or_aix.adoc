...
* You must have installed Java 17 (for SnapCenter 6.2.1 and later) or Java 11 (for SnapCenter 6.2 and earlier) on your Linux or AIX host.
** Java from Oracle, OpenJDK, or SAP JDK is supported for Linux.
** IBM Java for AIX. You can download from https://developer.ibm.com/languages/java/semeru-runtimes/downloads/?version=17&os=AIX[IBM Semeru Runtimes Downloads^].
+
...
a|
* Java 17 (Oracle Java, OpenJDK, or SAP JDK) for SnapCenter 6.2.1 and later or Java 11 for SnapCenter 6.2 and earlier
...
Required software packages
a|
* IBM Java 17 for SnapCenter 6.2.1 and later or IBM Java 11 for SnapCenter 6.2 and earlier
...
CAUTION: Although you are allowed to create credentials after deploying hosts and installing plug-ins, the *best practice* is to create credentials after you have added SVMs, and before you deploy hosts and install plug-ins.
...
NOTE: If you specify incorrect details while creating a credential, an error message is displayed. You must select *Cancel*, and then retry.
...
. Select image:../media/filter_icon.gif[filter icon], and then select the host name and the database type to filter the resources.
+
You can then select image:../media/filter_icon.gif[filter icon] to close the filter pane.

. Select the database, and then select *Database Settings* > *Configure Database*.
...
+
For more information, refer to link:../protect-sco/task_define_a_backup_strategy_for_oracle_databases.html#preferred-nodes-in-rac-setup[Preferred nodes in RAC setup^].

. Select *OK*.

// SNAPCDOC-394, 2026-APR-09
// SNAPCDOC-402, 2026-JUN-02
// GH issue #245, 2026-JUN-10